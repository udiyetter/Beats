...
const (
	...
	defaultBinaryName = "{{.Name}}{{if .Snapshot}}-SNAPSHOT{{end}}{{if .OS}}-{{.OS}}{{end}}{{if .Arch}}-{{.Arch}}{{end}}"
)
...
func (s PackageSpec) rootDir() string {
	...
	return s.MustExpand("{{.BeatName}}{{if .Snapshot}}-SNAPSHOT{{end}}{{if .OS}}-{{.OS}}{{end}}{{if .Arch}}-{{.Arch}}{{end}}")
}
